...
function routeToSemanticKey(route) {
...
function buildProviderInjectionScript(boot) {
...
function injectNamespaceProviderShell(html, boot) {
    ...
    if (!source.trim())
        return source;
    if (source.includes("__MONAD_PROVIDER_BOOT_INJECTED__"))
        return source;
    ...
    return {
        ...
        props: {
            sx: {
                ...
                p: 3,
                background: "radial-gradient(circle at top, rgba(33,150,243,0.12), transparent 45%), linear-gradient(180deg, rgba(9,14,24,1) 0%, rgba(16,24,38,1) 100%)",
            },
        },
        children: [
            {
                ...
                children: [
                    ...
                    },
                    {
                        ...
                        children: [
                            `namespace: ${input.namespace}\nroute: ${normalizeSurfaceRoute(input.route)}${rootName ? `\nroot: ${rootName}` : ""}${hostId ? `\nhost: ${hostId}` : ""}`,
                        ],
                    },
                ],
            },
