...
function normalizeNetGetEndpoint(value) {
    ...
    try {
        ...
        if (url.protocol !== "http:" && url.protocol !== "https:")
            return null;
        if (!LOCAL_NETGET_HOSTS.has(url.hostname))
            return null;
...
function buildRegistrationPayload(input) {
    ...
    return {
        ...
        updatedAt: now,
        ttlMs: Math.max(input.heartbeatMs * 4, 12000),
        mode: "fixed",
        portStatus: "active",
    };
}
export function startNetGetMonadRegistration(bootstrap, logger = null) {
    ...
    if (!baseEndpoint)
        return null;
    const heartbeatMs = Math.max(numberFromEnv("MONAD_NETGET_HEARTBEAT_MS", 3000), 1000);
