...
import { appendSemanticMemory, listHostMemoryHistory, listSemanticMemoriesByNamespace, listSemanticMemoriesByRootNamespace, } from "./src/claim/memoryStore.js";
import { filterBlocksByNamespace, formatObserverRelationLabel, resolveHostNamespace, resolveNamespace, resolveNamespaceProjectionRoot, resolveObserverRelation, resolveTransportHost, } from "./src/http/namespace.js";
...
import { parseBridgeTarget, buildBridgeTarget, buildNormalizedTarget, buildKernelCommandTarget, getNamespaceSelectorInfo, } from "./src/runtime/bridge.js";
import { normalizeOperation, normalizeClaimableNamespace, isCanonicalClaimableNamespace, resolveCommandNamespace, getDefaultReadPolicy, parseNamespaceIdentity, } from "./src/runtime/commands.js";
import { buildProviderBoot, createProviderSurface, } from "./src/surfaces/providerSurface.js";
...
const LOCAL_NAMESPACE_ROOT = normalizeNamespaceIdentity(SELF_NODE_CONFIG?.identity || process.env.ME_NAMESPACE || NODE_HOSTNAME);
const surfaceConfig = {
...
const semanticBootstrapRoot = normalizeNamespaceRootName(SELF_NODE_CONFIG?.identity || LOCAL_NAMESPACE_ROOT);
...
app.post("/me/*", async (req, res) => {
    ...
    if (operation === "claim") {
        ...
        if (!out.ok) {
            const status = out.error === "NAMESPACE_TAKEN" ? 409
    ...
    if (!out.ok) {
        const status = out.error === "CLAIM_NOT_FOUND" ? 404
...
app.post("/", async (req, res) => {
    ...
    const resolvedNamespace = resolveCommandNamespace(operation, (body ?? {}), parsedTarget, resolveNamespace(req));
...
app.get("/", (req, res) => {
    ...
    blocks = blocks.slice().sort((a, b) => Number(b?.timestamp || 0) - Number(a?.timestamp || 0)).slice(0, limit);
