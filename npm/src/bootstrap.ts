...
function resolveEnvValue(
  options: MonadOptions,
  env: NodeJS.ProcessEnv,
  key: keyof MonadOptions,
  envKey: string,
): string | undefined {
...
export function resolveMonadRuntimeConfig(options: MonadOptions = {}): MonadRuntimeConfig {
  ...
  const localNamespaceRoot = normalizeNamespaceIdentity(
    selfNodeConfig?.identity || env.ME_NAMESPACE || nodeHostname,
  );
