...
function createRoleSeeds(
  role: string,
  config: {
    behaviorType: "entity" | "collection" | "scalar";
    iterator?: string;
    contains?: string[];
  },
): SemanticSeed[] {
...
function createFieldSeeds(path: string, type: string, unit?: string): SemanticSeed[] {
  const seeds: SemanticSeed[] = [{ path: `schema.field.${path}.type`, data: type }];
  if (unit) {
...
const ROOT_ROLE_SEEDS: SemanticSeed[] = [
...
const ROOT_FIELD_SEEDS: SemanticSeed[] = [
...
const ROOT_GUI_DEFAULT_SEEDS: SemanticSeed[] = [
...
export const ROOT_SCHEMA_SEEDS: SemanticSeed[] = [
  ...ROOT_ROLE_SEEDS,
  ...ROOT_FIELD_SEEDS,
  ...ROOT_GUI_DEFAULT_SEEDS,
];

export const POLLS_STUDIO_DEFAULT_CATEGORIES: PollsStudioCategoryDefinition[] = [
  {
    slug: "community",
    label: "Community",
...
export function buildPollsStudioCategorySeeds(basePath = "polls.studio.categories"): SemanticSeed[] {
