...
import { findMonadByNameAsync, findMonadsForNamespaceAsync, listMonadIndexAsync } from "./monadIndex.js";
 ...
 */
function rootspaceOf(namespace) {
...
export async function selectMeshClaimantByScope(opts) {
    ...
    const candidates = all.filter((m) => m.endpoint.replace(/\/+$/, "") !== normSelf &&
    ...
    function claimsNamespace(m, ns) {
        if (m.namespace.trim().toLowerCase() === ns)
            return true;
